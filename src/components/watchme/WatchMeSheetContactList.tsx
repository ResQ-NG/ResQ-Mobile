  ...
  return (
    <View>
      <View
        ...
        style={{ backgroundColor: themeName === 'dark' ? '#153356' : '#E6F0FF' }}
      >
      ...
      <View style={{ paddingHorizontal: 16 }}>
        <View className="rounded-2xl overflow-hidden bg-surface-light dark:bg-surface-dark">
          {contacts.map((contact, index) => (
            <View key={contact.id} className="flex-row items-center gap-3 px-4 py-3.5">
              <Avatar
                size={36}
              ...
              <View className="flex-1 min-w-0">
                <AppText variant="body" className="font-metropolis-medium" numberOfLines={1}>
                  {contact.name}
                </AppText>
                <AppText variant="caption" className="text-sm mt-0.5" numberOfLines={1}>
                  {contact.phone}
                </AppText>
              </View>
              <TouchableOpacity
              ...
              >
                <SolarTrashBin2BoldDuotoneIcon width={22} height={22} color={theme.accentRed} />
              </TouchableOpacity>
            </View>
          ))}
        </View>
